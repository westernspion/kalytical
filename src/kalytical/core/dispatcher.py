...
class KDispatcher():
    ...
    async def dispatch(self, lifecycle_event: LifecycleEventModel) -> List(RunningPipelineModel): #TODO Generalize to any event
        self.log.info(f'{lifecycle_event.event_type} event received with event_body={lifecycle_event.event_body}')
    ...
    async def _handle_job_success_event(self, event_body: JobLifecycleEventBody) -> List[RunningPipelineModel]:
        ...
        if len(header_models) == 0:
            self.log.info(f"Success event for pipeline_uuid={event_body.pipeline_uuid} has no descendants. Nothing to do!")
            return []
        ...
        for header_model in header_models:
            if(header_model.triggers_on is None) or ((header_model.triggers_on.operator == 'any') or ((header_model.triggers_on.operator == 'all') and (len(header_model.triggers_on.pipeline_uuids) == 1))):
                ...
                self._data_provider.update_incubating_jobs(trigger_pipeline_uuid=event_body.pipeline_uuid, header_model=header_model, source_uuid=event_body.exec_uuid)
        return submitted
    ...
    async def _handle_job_failure_event(self, event_body: JobLifecycleEventBody) -> List[RunningPipelineModel]:
        ...
        if int(event_body.retry_count) >= int(header_model.retry_max):
            raise MaxPipelineRetryReachedException(f"pipeline_uuid={event_body.pipeline_uuid} has reached maximum retry_count. Current retry_count={event_body.retry_count}")
        else:
            next_retry_count = int(event_body.retry_count) + 1
            self.log.warning(f"This is an attempt to retry a pipeline. New retry_count={next_retry_count}")
    ...
    async def _handle_job_origination_event(self, event_body: JobLifecycleEventBody) -> List[RunningPipelineModel]:
        ...
        if header_model is None:
            self.log.warning(f"Origination event for pipeline_uuid={event_body.pipeline_uuid} received, but it does not exist. Nothing to do!")
            return []
    ...
    async def queue_pipeline(self, header_model: PipelineHeader_model, retry_count: int = 0, source_uuids: Dict[str, str] = None) -> RunningPipelineModel:
        if not header_model.concurrency and await self._check_concurrency(pipeline_uuid=header_model.piipeline_uuid):
            self.log.warning(f"Attempted to schedule pipeline_uuid{header_model.pipeline_u9uid} but failed concurrency check. Deferring request to run job until excisting pipeline_uuid={header_model.pipelin_uuid} has completed.")
            self._data_provider.defer_job(pipeline_uuid=header_model.pipeline_uuid, created_by_uuid=source_uuids, reason='concurrency', trigger_model=None, retry_count=retry_count)
        ...
        event_body = JobLifecycleEventBody(exec_uuid=submitted_job.exec_uuid, source_uuids=source_uuids, pipeline_uuid=submitted_job.pipeline_uuid, retry_count=retry_count, event_time=datetime.now(), event_subtype='submitted')
        await self._data_provider.save_event_to_history(LifecycleEventModel(event_type="job_exec_update", event_body=event_body))
    ...
    async def _check_concurrency(self, pipeline_uuid: str) -> bool:
        ...
        if pipeline_uuid in running_pipeline_uuids:
            self.log.warning(f"Concurrency for pipeline_uuid={pipeline_uuid} is not supported!") 
            return True
        return False
